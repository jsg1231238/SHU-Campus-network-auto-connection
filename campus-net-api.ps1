...
#  依据反向工程 上网认证.html + AuthInterFace.js + login_bch.js + security.js
#
#  首次使用先存密码（在将来运行计划任务的那个 Windows 账号下执行一次）：
#    (Read-Host '校园网密码' -AsSecureString | ConvertFrom-SecureString) |
#        Set-Content (Join-Path $PSScriptRoot '.campuspwd') -Encoding ASCII
#  DPAPI 按“当前用户”加密，所以计划任务必须以同一账号运行（见文件末尾注册命令）。
# ============================================================
param(
    [string]$WifiSSID   = "改成你的WiFi名字",              # ← 必改（或用下面的 local.settings.ps1 覆盖）
    [string]$Account    = "改成你的校园网账号(学号)",       # ← 必改（或用下面的 local.settings.ps1 覆盖）
    [string]$Password   = "",                         # 留空则读 .campuspwd(DPAPI)，不要在这里写明文密码
    [string]$Service    = "shu",                      # 抓包确认：这所学校=shu，换学校要重新抓包确认
    [string]$PortalHost = "10.10.9.9",
    # RSA 公钥（已与浏览器逐字节验证一致；若某天登录突然全部"密码错误"，可能是服务器换了公钥，
    # 到认证页 F12 读 document.getElementById('publicKeyModulus').value 更新这里即可）
    [string]$PubExp     = "10001",
...
# 本地私有配置覆盖：把你的真实 WiFi 名/账号放进同目录下的 local.settings.ps1（不进 Git，见 .gitignore），
# 这样公开发布的脚本本体不含任何个人信息，你自己的计划任务也不用改一行调用方式。
...
for ($i = 1; $i -le $MaxRetry; $i++) {
    ...
    # 登录失败响应里 validCodeUrl 非空 = 服务器要求验证码（见 login_bch.js:847），无人值守输不了，重试无意义
...
# ============================================================
#  注册计划任务（管理员 PowerShell，一次性）：登录时 + 每 30 分钟轮询一次。
#  说明：校园网每天掉线时 WiFi 并不断开，不会触发"联网事件"，所以用定时轮询兜底；
#        脚本开头有 Test-Online 预检，已在线会立刻退出，轮询开销很小。
#
#  $vbs = "E:\claude-work\20260709_netconnect\launch-hidden.vbs"   # ← 写 VBS 完整路径（交互式窗口里 $PSScriptRoot 是空的，不能用）
#  $act = New-ScheduledTaskAction -Execute "wscript.exe" -Argument "//B //Nologo `"$vbs`""
#  （不直接跑 powershell.exe：控制台程序在计划任务里必弹黑窗、Win11 下还抢前台；
#    wscript 是 GUI 程序，经 launch-hidden.vbs 以“创建即隐藏”方式启动 powershell，全程无窗口。）
...
#
#  ↑ 默认"仅当前用户登录时运行"（锁屏也算登录，正常够用；DPAPI 也只有此账号能解密）。
#  若需要"注销/未登录也运行"，在 Register 末尾加： -User "$env:USERDOMAIN\$env:USERNAME" -LogonType Password
#  （会要求输入 Windows 登录密码，供任务计划保存）。
# ============================================================
